...
export function SearchLayout() {
  ...
  const commonTooltipContentClass = `
    ...
    `}>
      <div className="flex items-center justify-between">
        ...
        <TooltipProvider delayDuration={300}>
          <Tooltip>
            ...
            <TooltipContent side="bottom" sideOffset={5} className={commonTooltipContentClass}>
              <p>Trang chủ</p>
            </TooltipContent>
          </Tooltip>
        </TooltipProvider>
      ...
      <div className="flex-1 max-w-2xl mx-4">
        <div className="relative">
          <Search className={`absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 ${isDarkMode ? 'text-dark-sectionHeader' : 'text-light-sectionHeader'}`} />
          <Input
            type="search"
            placeholder="Tìm kiếm..."
            className={`
              pl-10 pr-4 py-2 rounded-full border focus-visible:ring
              ${isDarkMode ? 'bg-dark-hoverBg border-dark-divider focus-visible:ring-dark-activeText' : 'bg-light-hoverBg border-light-divider focus-visible:ring-light-activeText'}
            `}
          />
        </div>
      </div>
