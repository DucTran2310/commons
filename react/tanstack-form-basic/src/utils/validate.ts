...
export const validatePassword = ({ value }: { value: string }) => {
  ...
  if (value.length < 8) errors.push("Tối thiểu 8 ký tự");
  if (!/[A-Z]/.test(value)) errors.push("Chứa ít nhất 1 chữ hoa");
  if (!/[a-z]/.test(value)) errors.push("Chứa ít nhất 1 chữ thường");
  if (!/[0-9]/.test(value)) errors.push("Chứa ít nhất 1 số");
  if (!/[^A-Za-z0-9]/.test(value)) errors.push("Chứa ít nhất 1 ký tự đặc biệt");
